...
st.set_page_config(
    page_title="Pareto Falhas QG09 - V0.7",
    page_icon="📊",
    layout="wide",
...
def apply_filters(df, modelos, origens, areas, period):
    out = df.copy()
...
<div class='hero'>
  ...
  <p>Versão {APP_VERSION}: CD_POSTO_FALHA como filtro QG09 e DT_CRIACAO_FALHA como data.</p>
  <span class='badge'>Filtro: CD_POSTO_FALHA = QG09</span><span class='badge'>Sem Plotly</span><span class='badge'>Cabeçalhos flexíveis</span><span class='badge'>Correção automática de modelos</span>
</div>
...
with tabs[3]:
    st.markdown("<div class='panel'><div class='panel-title'>Base & Upload</div><div class='panel-sub'>A V0.7 reconhece CD_POSTO_FALHA e DT_CRIACAO_FALHA automaticamente.</div>", unsafe_allow_html=True)
...
if df.empty:
    ...
    with st.sidebar:
        ...
        areas = sorted([x for x in df["C_AREA_ORIGEM_FALHA"].dropna().unique().tolist() if x])
    ...
    filt = apply_filters(df, modelos_sel, origens_sel, areas_sel, period)
...
with tabs[4]:
    st.markdown("<div class='panel'><div class='panel-title'>Sobre</div><div class='panel-sub'>Versão V0.7 ajustada para a base com CD_POSTO_FALHA e DT_CRIACAO_FALHA.</div>", unsafe_allow_html=True)
    st.markdown("""
### Regras do site
- Filtra automaticamente `CD_POSTO_FALHA = QG09`.
- Usa `DT_CRIACAO_FALHA` como data principal do Pareto.
- Usa `ANOMALIA_FALHA` para montar o Pareto principal.
- Remove linhas sem falha preenchida.
- Cria `MODELO_CORRIGIDO` automaticamente.

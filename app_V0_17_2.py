...
st.set_page_config(page_title="AGCO | Pareto QG09 V0.17.2", page_icon="📊", layout="wide")
APP_VERSION = "V0.17.2"
DB = "pareto_qg09_v017_2.db"
POSTO_FIXO = "QG09"
...
if df_all.empty or year is None:
    ...
    with tabs[2]:
        ...
        if len(mp)==1:
            ...
            dfmp=filt.copy() if m=='Todos' else filt[filt.MODELO_CORRIGIDO.eq(m)].copy()
            titulo_modelo='Todos os modelos' if m=='Todos' else m
            pm=pareto(dfmp,'D1_GERAL',top_n)
            x,y,z=st.columns(3)
            x.markdown(kpi('Modelo',titulo_modelo,'Selecionado'),unsafe_allow_html=True)
            y.markdown(kpi('Falhas',fint(len(dfmp)),'No recorte'),unsafe_allow_html=True)
            z.markdown(kpi('Tipos de falha',fint(dfmp.D1_GERAL.nunique()),'D1_GERAL'),unsafe_allow_html=True)
            st.markdown(pareto_svg(pm,f'Pareto por Modelo - {titulo_modelo} - {label}'),unsafe_allow_html=True)
            ps=pm.copy(); ps.insert(0,'TOP',range(1,len(ps)+1))
            st.dataframe(add_percent_cols(ps),use_container_width=True,hide_index=True)
    ...
    with tabs[4]:
        ...
        if filt.empty: st.info('Sem dados.')
            ...
            combo=filt.copy()
            combo['MODELO_X_FALHA']=combo['MODELO_CORRIGIDO'].astype(str)+' / '+combo['D1_GERAL'].astype(str)
            pmat=pareto(combo,'MODELO_X_FALHA',top_n)
            st.markdown(pareto_svg(pmat, f'Pareto Modelo x Falha - {label}'), unsafe_allow_html=True)
            pmat_show=pmat.copy()
            if not pmat_show.empty:
                pmat_show.insert(0,'TOP',range(1,len(pmat_show)+1))
                pmat_show=pmat_show.rename(columns={'Item':'Modelo / Falha'})
            st.dataframe(add_percent_cols(pmat_show), use_container_width=True, hide_index=True)

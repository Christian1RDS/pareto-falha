...
st.set_page_config(page_title="Pareto QG09 V0.13", page_icon="📊", layout="wide")
APP_VERSION="V0.13"; DB="pareto_qg09_v013.db"; POSTO_FIXO="QG09"
...
CSS="""
<style>
:root{--agco-red:#C00031;--agco-red2:#C41230;--agco-black:#241F20;--agco-bg:#F4F5F7;--agco-card:#FFFFFF;--agco-border:#D6D9DE;--agco-muted:#657080;--agco-text:#23272F;}
.stApp{background:linear-gradient(180deg,#f4f5f7 0%,#e9ecef 100%);color:var(--agco-text);}
[data-testid="stSidebar"]{background:linear-gradient(180deg,#241F20 0%,#171416 100%);border-right:4px solid var(--agco-red);}
[data-testid="stSidebar"] *{color:#f5f5f5 !important;}.block-container{padding-top:1.1rem;}
.hero{padding:24px 28px;border:1px solid var(--agco-border);background:linear-gradient(135deg,#fff 0%,#f7f8fa 65%,#feeef2 100%);border-radius:12px;box-shadow:0 12px 28px rgba(36,31,32,.12);margin-bottom:18px;border-top:6px solid var(--agco-red)}
.hero h1{margin:0;color:var(--agco-black);font-weight:900}.hero p{color:var(--agco-muted);margin:8px 0 0}.badge{display:inline-block;padding:5px 10px;border-radius:4px;border:1px solid #e6b3bf;background:#fff;color:var(--agco-red);font-weight:800;font-size:.80rem;margin-right:6px;margin-top:8px;text-transform:uppercase;letter-spacing:.3px}
.panel{background:#fff;border:1px solid var(--agco-border);border-left:5px solid var(--agco-red);border-radius:10px;padding:18px;box-shadow:0 10px 24px rgba(36,31,32,.10);margin-bottom:16px}.kpi{background:#fff;border:1px solid var(--agco-border);border-top:4px solid var(--agco-red);border-radius:10px;padding:18px;min-height:115px;box-shadow:0 8px 20px rgba(36,31,32,.08)}.kpi-label{color:var(--agco-muted);font-size:.80rem;font-weight:900;text-transform:uppercase;letter-spacing:.6px}.kpi-value{color:var(--agco-black);font-size:1.7rem;font-weight:900;margin-top:8px}.kpi-sub{color:var(--agco-muted);font-size:.85rem}.pareto-box{background:#fff;border:1px solid var(--agco-border);border-radius:10px;padding:14px;overflow-x:auto}.small-note{color:var(--agco-muted);font-size:.9rem;background:#fff;border:1px solid var(--agco-border);border-left:5px solid var(--agco-red);padding:10px 12px;border-radius:8px;margin-bottom:12px}svg text{font-family:Arial,Helvetica,sans-serif}
</style>
"""
...
def svg_bar(p,title,ordered=True):
    if p.empty: return "<div class='pareto-box'>Sem dados.</div>"
    p=p.copy().reset_index(drop=True)
    w,h,ml,mr,mt,mb=1180,530,70,45,55,155; pw=w-ml-mr; ph=h-mt-mb; mx=max(p.Quantidade.max(),1); step=pw/max(len(p),1); bw=min(step*.68,72); parts=[f"<div class='pareto-box'><svg viewBox='0 0 {w} {h}' width='100%' height='{h}'>","<rect width='100%' height='100%' fill='#FFFFFF'/>",f"<text x='{ml}' y='32' fill='#241F20' font-size='22' font-weight='800'>{html.escape(title)}</text>"]
    pts=[]
    for i,r in p.iterrows():
        x=ml+step*i+step/2; bh=(r.Quantidade/mx)*ph; y=mt+ph-bh; lab=str(r.Item); short=lab[:26]+('...' if len(lab)>26 else '')
        parts += [f"<rect x='{x-bw/2:.1f}' y='{y:.1f}' width='{bw:.1f}' height='{bh:.1f}' fill='#C00031' rx='3'><title>{html.escape(lab)} - {int(r.Quantidade)}</title></rect>",f"<text x='{x:.1f}' y='{max(y-7,45):.1f}' fill='#241F20' font-size='12' text-anchor='middle'>{int(r.Quantidade)}</text>",f"<text x='{x:.1f}' y='{mt+ph+24}' fill='#657080' font-size='11' text-anchor='end' transform='rotate(-35 {x:.1f} {mt+ph+24})'>{html.escape(short)}</text>"]
...
if df_all.empty or year is None:
    ...
    with tabs[1]:
        if pg.empty: st.info('Sem dados.')
            ...
            if not det.empty:
                det.insert(0,'Ranking',range(1,len(det)+1)); det['Qtd']=det.Quantidade; det['Descrição ranking']=det.apply(lambda r:f"{int(r.Ranking)} - {r['Falha completa']} (qtd-{int(r.Qtd)})",axis=1); det['Percentual']=det.Percentual.map(fpct); det['Percentual Acumulado']=det['Percentual Acumulado'].map(fpct)
            ...
            if not det.empty:
                g=det.head(10).rename(columns={'Falha completa':'Item','Qtd':'Quantidade'}).copy(); total=g.Quantidade.sum(); g['Percentual']=g.Quantidade/total if total else 0; g['Percentual Acumulado']=g.Percentual.cumsum(); st.markdown(svg_bar(g[['Item','Quantidade','Percentual','Percentual Acumulado']],'Top 10 falhas completas - ordenado por quantidade'),unsafe_allow_html=True)

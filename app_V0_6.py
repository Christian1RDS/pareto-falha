...
st.set_page_config(
    page_title="Pareto Falhas QG09 - V0.6",
    page_icon="📊",
    layout="wide",
...
ALIASES = {
    "CD_POSTO_CN": ["CD_POSTO_CN", "CD\\_POSTO\\_CN", "POSTO", "POSTO_CN", "CD_POSTO", "CDPOSTOCN"],
    "CD_MODELO": ["CD_MODELO", "CD\\_MODELO", "MODELO", "COD_MODELO", "CODIGO_MODELO", "CDMODELO"],
    "DT_HR_INSPECAO": ["DT_HR_INSPECAO", "DT\\_HR\\_INSPECAO", "DATA_INSPECAO", "DT_INSPECAO", "DATA_HORA_INSPECAO", "DATA", "DTHRINSPECAO"],
    "ANOMALIA_FALHA": ["ANOMALIA_FALHA", "ANOMALIA\\_FALHA", "FALHA", "ANOMALIA", "DESCRICAO_FALHA", "DESC_FALHA"],
    "NR_WO": ["NR_WO", "NR\\_WO", "WO", "ORDEM", "NR_ORDEM"],
    "NR_SERIE": ["NR_SERIE", "NR\\_SERIE", "SERIE", "CHASSI", "NR_CHASSI"],
    "POSTO_ORIGEM_FALHA": ["POSTO_ORIGEM_FALHA", "POSTO\\_ORIGEM\\_FALHA", "ORIGEM_FALHA", "POSTO_ORIGEM", "ORIGEM"],
    "CD_USER_INSPECAO": ["CD_USER_INSPECAO", "CD\\_USER\\_INSPECAO", "USUARIO", "USER_INSPECAO", "INSPETOR"],
    "C_DPU_QG_AMARELO": ["C_DPU_QG_AMARELO", "C\\_DPU\\_QG\\_AMARELO", "DPU", "DPU_QG_AMARELO"],
}
...
<div class='hero'>
  ...
  <p>Versão {APP_VERSION}: leitura flexível de cabeçalhos + Pareto clássico sem Plotly.</p>
  <span class='badge'>Filtro fixo: QG09</span><span class='badge'>Sem Plotly</span><span class='badge'>Cabeçalhos flexíveis</span><span class='badge'>Correção automática de modelos</span>
</div>
...
with tabs[3]:
    st.markdown("<div class='panel'><div class='panel-title'>Base & Upload</div><div class='panel-sub'>A V0.6 aceita cabeçalhos como CD\\_POSTO\\_CN, CD POSTO CN, Dt Hr Inspecao etc.</div>", unsafe_allow_html=True)
    ...
    if uploaded:
        try:
            ...
            if not ok:
                ...
                st.warning("Confira no diagnóstico acima como o site leu os nomes das colunas.")
            else:
...
if df.empty:
    with tabs[0]:
        st.info("Faça upload da base na aba Base & Upload.")
    with tabs[1]:
        st.info("Faça upload da base na aba Base & Upload.")
    with tabs[2]:
        st.info("Faça upload da base na aba Base & Upload.")
...
with tabs[4]:
    st.markdown("<div class='panel'><div class='panel-title'>Sobre</div><div class='panel-sub'>Versão V0.6 com leitura flexível de cabeçalhos e sem dependência de Plotly.</div>", unsafe_allow_html=True)
    st.markdown("""
### Regras do site

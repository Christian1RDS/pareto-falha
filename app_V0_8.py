...
st.set_page_config(
    page_title="Pareto Falhas QG09 - V0.8",
    page_icon="📊",
    layout="wide",
...
def read_file(uploaded):
    ...
    if ext == "csv":
        ...
        for enc in ["utf-8-sig", "utf-16", "latin1"]:
            for sep in [None, "\t", ";", ","]:
                try:
                    raw = pd.read_csv(io.BytesIO(content), encoding=enc, sep=sep, engine="python" if sep is None else "c", dtype=str)
...
def prepare(raw):
    ...
    cols = ["CD_POSTO_CN", "NR_WO", "NR_SERIE", "CD_MODELO", "MODELO_CORRIGIDO", "DT_HR_INSPECAO", "ANOMALIA_FALHA", "POSTO_ORIGEM_FALHA", "C_AREA_ORIGEM_FALHA"]
...
def save_upload(conn, file_name, full, qg09, falhas, mode):
    ...
    conn.executemany("""
        INSERT INTO falhas_qg09 (upload_id, nr_wo, nr_serie, cd_modelo, modelo_corrigido, dt_hr_inspecao, anomalia_falha, posto_origem_falha, c_area_origem_falha)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, rows)
    conn.commit()
    return uid
...
def load_calendar(conn):
    ...
    # Evita duplicidade se o mesmo arquivo for salvo mais de uma vez.
...
<div class='hero'>
  ...
  <p>Versão {APP_VERSION}: calendário acumulativo 2025/2026, modos Diário/Semanal/Mensal/YTD/Personalizado e filtro fixo QG09.</p>
  <span class='badge'>Somar ao calendário</span><span class='badge'>Apenas QG09</span><span class='badge'>YTD</span><span class='badge'>Personalizado</span>
</div>
...
if df_all.empty or year is None:
    ...
    with tabs[0]:
        ...
        st.markdown(f"<div class='small-note'>Posto considerado: <b>QG09</b> | Ano: <b>{year}</b> | Recorte: <b>{period_label}</b></div>", unsafe_allow_html=True)
        ...
        st.markdown(pareto_svg(pareto, f"Pareto de Falhas QG09 - {period_label} - Top {top_n}"), unsafe_allow_html=True)
        show = pareto.copy()
        if not show.empty:
    ...
    with tabs[1]:
        ...
        st.markdown(pareto_svg(p, f"Pareto - {modelo} - {period_label} - Top {top_n}"), unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    with tabs[2]:
...
with tabs[5]:
    st.markdown("<div class='panel'><div class='panel-title'>Sobre</div><div class='panel-sub'>Versão V0.8 com calendário acumulativo e modos de período.</div>", unsafe_allow_html=True)
    st.markdown("""
### Regras da V0.8
- Considera somente falhas cujo posto é `QG09`.
- Permite salvar 2025 e 2026 no mesmo calendário usando `Somar ao calendário`.
- Tem modos `Diário`, `Semanal`, `Mensal`, `Anual YTD` e `Personalizado`.
- `Personalizado` mantém a seleção manual de período.
- `POSTO_ORIGEM_FALHA` é apenas a origem da falha dentro dos registros QG09.
""")
    st.markdown("</div>", unsafe_allow_html=True)

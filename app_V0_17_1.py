...
st.set_page_config(page_title="AGCO | Pareto QG09 V0.17.1", page_icon="📊", layout="wide")
APP_VERSION = "V0.17.1"
DB = "pareto_qg09_v017_1.db"
POSTO_FIXO = "QG09"
...
db=con(); init(db)
st.markdown(f"<div class='agco-header'>{logo_html()}<div class='head-title'><h1>Pareto de Falhas QG09</h1><p>Versão {APP_VERSION}: V0.17 base com apenas dois ajustes pedidos.</p><span class='badge'>Base V0.17</span><span class='badge'>Modelo Todos</span><span class='badge'>Pareto na matriz</span></div></div>",unsafe_allow_html=True)
df_all=load(db)
tabs=st.tabs(['Dashboard','Estratificar TOP','Pareto por Modelo','Região/Estrutura','Matriz Modelo x Falha','Tendência Semanal','Antes x Depois','Famílias de Defeito','Upload','Histórico'])
with st.sidebar:
...
with tabs[8]:
    st.markdown("<div class='panel'><b>Upload</b><br>Base V0.17. Apenas adicionado opção Todos no Pareto por Modelo e Pareto na Matriz.</div>",unsafe_allow_html=True)
...
if df_all.empty or year is None:
    ...
    with tabs[2]:
        mp=['Todos']+sorted(filt.MODELO_CORRIGIDO.dropna().unique())
        if len(mp)==1: st.info('Sem modelos disponíveis.')
        else:
            m=st.selectbox('Modelo para Pareto',mp)
            base_modelo=filt.copy() if m=='Todos' else filt[filt.MODELO_CORRIGIDO.eq(m)].copy()
            titulo_modelo='Todos os modelos' if m=='Todos' else m
            pm=pareto(base_modelo,'D1_GERAL',top_n)
            st.markdown(pareto_svg(pm,f'Pareto por Modelo - {titulo_modelo} - {label}'),unsafe_allow_html=True); show_table(pm)
    with tabs[3]:
    ...
    with tabs[4]:
        st.markdown("<div class='panel'><b>Matriz Modelo x Falha</b><br>Agora esta aba também tem Pareto clássico das combinações Modelo x Falha.</div>",unsafe_allow_html=True)
        if filt.empty: st.info('Sem dados.')
        else:
            top_f=filt.D1_GERAL.value_counts().head(15).index.tolist()
            mat=pd.pivot_table(filt[filt.D1_GERAL.isin(top_f)],index='MODELO_CORRIGIDO',columns='D1_GERAL',values='ANOMALIA_FALHA',aggfunc='count',fill_value=0)
            st.dataframe(mat,use_container_width=True)
            combo=filt.copy()
            combo['COMBINACAO_MODELO_FALHA']=combo['MODELO_CORRIGIDO'].astype(str)+' / '+combo['D1_GERAL'].astype(str)
            pc=pareto(combo,'COMBINACAO_MODELO_FALHA',top_n)
            st.markdown(pareto_svg(pc,f'Pareto Modelo x Falha - {label}'),unsafe_allow_html=True); show_table(pc.rename(columns={'Item':'Modelo / Falha'}))
    with tabs[5]:
        if filt.empty: st.info('Sem dados.')
        else:

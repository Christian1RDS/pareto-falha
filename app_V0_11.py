...
st.set_page_config(page_title="Pareto QG09 V0.11", page_icon="📊", layout="wide")
APP_VERSION = "V0.11"
DB = "pareto_qg09_v011.db"
POSTO_FIXO = "QG09"
...
st.markdown(f"<div class='hero'><h1>Pareto de Falhas QG09</h1><p>Versão {APP_VERSION}: TOP com opção Todos, ranking completo e Pareto por Modelo.</p><span class='badge'>D1_GERAL</span><span class='badge'>Todos</span><span class='badge'>Pareto por Modelo</span><span class='badge'>QG09 fixo</span></div>", unsafe_allow_html=True)

df_all=load_calendar(c)
tabs=st.tabs(["Dashboard", "Estratificar TOP", "Pareto por Modelo", "Upload", "Histórico"])
...
if df_all.empty or year is None:
    ...
    with tabs[2]:
        st.markdown("<div class='panel'><b>Pareto por Modelo</b><br>Escolha um modelo e veja o Pareto de falha geral dentro dele.</div>", unsafe_allow_html=True)
        modelos_pareto = sorted(filt.MODELO_CORRIGIDO.dropna().unique())
        if not modelos_pareto:
            st.info("Sem modelos disponíveis no recorte atual.")
        else:
            modelo_pareto = st.selectbox("Modelo para Pareto", modelos_pareto)
            df_modelo_pareto = filt[filt.MODELO_CORRIGIDO.eq(modelo_pareto)].copy()
            p_modelo = make_pareto(df_modelo_pareto, "D1_GERAL", top_n)
            cpm1, cpm2, cpm3 = st.columns(3)
            cpm1.markdown(kpi("Modelo", modelo_pareto, "Selecionado"), unsafe_allow_html=True)
            cpm2.markdown(kpi("Falhas", fmt_int(len(df_modelo_pareto)), "No recorte"), unsafe_allow_html=True)
            cpm3.markdown(kpi("Tipos de falha", fmt_int(df_modelo_pareto.D1_GERAL.nunique()), "D1_GERAL"), unsafe_allow_html=True)
            st.markdown(simple_svg(p_modelo, f"Pareto por Modelo - {modelo_pareto} - {label}"), unsafe_allow_html=True)
            p_show = p_modelo.copy()
            if not p_show.empty:
                p_show.insert(0, "TOP", range(1, len(p_show)+1))
                p_show["Percentual"] = p_show["Percentual"].map(fmt_pct)
                p_show["Percentual Acumulado"] = p_show["Percentual Acumulado"].map(fmt_pct)

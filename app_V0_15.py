...
import io, re, sqlite3, unicodedata, html, os, base64
from datetime import datetime, date, time, timedelta
from calendar import monthrange
import pandas as pd
import streamlit as st

st.set_page_config(page_title="AGCO | Pareto QG09 V0.15", page_icon="📊", layout="wide")
APP_VERSION = "V0.15"
DB = "pareto_qg09_v015.db"
POSTO_FIXO = "QG09"
...
<style>
:root{
  --agco-red:#C00031; --agco-red-2:#C41230; --agco-black:#241F20;
  --agco-bg:#101113; --agco-panel:#181A1D; --agco-panel-2:#202327;
  --agco-border:#3A3D42; --agco-muted:#B7BDC6; --agco-text:#F4F5F7;
}
.stApp{background:linear-gradient(180deg,#101113 0%,#17191C 100%);color:var(--agco-text);} 
[data-testid="stSidebar"]{background:linear-gradient(180deg,#241F20 0%,#151315 100%);border-right:5px solid var(--agco-red);} 
[data-testid="stSidebar"] *{color:#fff!important}.block-container{padding-top:1.0rem;}
.agco-header{display:flex;gap:20px;align-items:center;padding:22px 26px;border:1px solid var(--agco-border);background:linear-gradient(135deg,#241F20 0%,#181A1D 62%,#3a0b16 100%);border-radius:12px;box-shadow:0 14px 34px rgba(0,0,0,.30);border-top:6px solid var(--agco-red);margin-bottom:18px;}
.logo-box{background:#fff;padding:8px 12px;border-radius:4px;display:flex;align-items:center;justify-content:center;min-width:145px;min-height:58px;}
.brand-word{font-size:2.15rem;font-weight:950;letter-spacing:.8px;color:#241F20;line-height:1}.brand-sub{font-size:.72rem;color:#241F20;text-transform:uppercase;letter-spacing:1.4px;margin-top:4px}.head-title h1{margin:0;color:#fff;font-size:2.0rem}.head-title p{color:var(--agco-muted);margin:6px 0 0}.badge{display:inline-block;padding:5px 10px;border-radius:4px;border:1px solid rgba(255,255,255,.18);background:rgba(192,0,49,.22);color:#fff;font-weight:800;font-size:.78rem;margin-right:6px;margin-top:8px;text-transform:uppercase}.panel{background:var(--agco-panel);border:1px solid var(--agco-border);border-left:5px solid var(--agco-red);border-radius:10px;padding:18px;box-shadow:0 10px 24px rgba(0,0,0,.25);margin-bottom:16px}.kpi{background:linear-gradient(180deg,var(--agco-panel-2),var(--agco-panel));border:1px solid var(--agco-border);border-top:4px solid var(--agco-red);border-radius:10px;padding:18px;min-height:115px}.kpi-label{color:var(--agco-muted);font-size:.80rem;font-weight:900;text-transform:uppercase;letter-spacing:.6px}.kpi-value{color:#fff;font-size:1.55rem;font-weight:900;margin-top:8px}.kpi-sub{color:var(--agco-muted);font-size:.85rem}.pareto-box{background:#fff;border:1px solid var(--agco-border);border-radius:10px;padding:14px;overflow-x:auto}.small-note{color:#fff;background:var(--agco-panel);border:1px solid var(--agco-border);border-left:5px solid var(--agco-red);padding:10px 12px;border-radius:8px;margin-bottom:12px}svg text{font-family:Arial,Helvetica,sans-serif}.stDataFrame{background:#fff!important}
</style>
"""
...
def logo_html():
    if os.path.exists(LOGO_FILE):
        ...
        return f"<div class='logo-box'><img src='data:image/png;base64,{b64}' style='max-height:58px;max-width:180px;'/></div>"
...
def pareto_svg(p,title):
    ...
    for k in range(6):
        ...
        parts += [f"<line x1='{ml}' y1='{y:.1f}' x2='{w-mr}' y2='{y:.1f}' stroke='#E2E5E9' stroke-width='1'/>",f"<text x='{ml-12}' y='{y+4:.1f}' fill='#657080' font-size='12' text-anchor='end'>{int(round(q))}</text>",f"<text x='{w-mr+12}' y='{yp(pct)+4:.1f}' fill='#657080' font-size='12'>{int(pct*100)}%</text>"]
...
st.markdown(f"<div class='agco-header'>{logo_html()}<div class='head-title'><h1>Pareto de Falhas QG09</h1><p>Versão {APP_VERSION}: Pareto clássico com linha de 80% e visual inspirado no site corporativo da AGCO.</p><span class='badge'>D1_GERAL</span><span class='badge'>Pareto 80%</span><span class='badge'>Logo oficial</span><span class='badge'>QG09 fixo</span></div></div>",unsafe_allow_html=True)

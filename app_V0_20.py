...
st.set_page_config(page_title="AGCO | Pareto QG09 V0.20", page_icon="📊", layout="wide")
APP_VERSION="V0.20"
DB="pareto_qg09_v020.db"
...
st.markdown('<style>.stApp{background:#101113;color:#f4f5f7}[data-testid="stSidebar"]{background:#241F20;border-right:5px solid #C00031}.box{background:#181A1D;border-left:5px solid #C00031;border-radius:10px;padding:14px;margin:10px 0}</style>', unsafe_allow_html=True)
...
def pareto(df,col,n=10):
    ...
    return pareto_from_series(s,n)
def pareto_from_series(s,n=15):
...
c=con(); init(c)
st.title('Pareto de Falhas QG09 - V0.20')
st.caption('Correção NameError: pareto_from_series + opções Semanais/Alertas/Foco')
df_all=load(c)
...
if df_all.empty or year is None:
    ...
    with tabs[0]:
        ...
        with b: st.markdown('<div class="box"><b>Foco recomendado da semana</b><br>Modelo + Região + Falha com maior volume na última semana.</div>',unsafe_allow_html=True); st.dataframe(foco(filt),use_container_width=True,hide_index=True)
    with tabs[1]:
    ...
    with tabs[5]:
        st.markdown('<div class="box"><b>Criticidade e estabilidade</b><br>Filtro abaixo mostra exatamente quais falhas estão em queda, crescimento, instáveis, estáveis ou sem histórico.</div>',unsafe_allow_html=True)
        if not crit.empty:

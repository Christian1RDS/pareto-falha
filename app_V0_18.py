...
st.set_page_config(page_title="AGCO | Pareto QG09 V0.18", page_icon="📊", layout="wide")
APP_VERSION="V0.18"
...
def enrich(df):
    ...
    df=df.copy(); df['REGIAO_EXTRAIDA']=df.ANOMALIA_FALHA.map(extrai_regiao); df['FAMILIA_DEFEITO']=df.apply(lambda r: familia_defeito(r.get('D1',''),r.get('ANOMALIA_FALHA','')),axis=1); df['SEMANA_INICIO']=df.DT_HR_INSPECAO.dt.to_period('W-MON').apply(lambda r: r.start_time.date() if pd.notna(r.start_time) else None); return df
...
def prepare(raw):
    ...
    df.CD_POSTO_CN=df.CD_POSTO_CN.map(lambda x:'QG09' if 'QG09' in txt(x).upper() else txt(x).upper()); df.CD_MODELO=df.CD_MODELO.map(lambda x:txt(x).upper()); df['MODELO_CORRIGIDO']=df.CD_MODELO.map(corr_modelo); df.DT_HR_INSPECAO=parse_dt(df.DT_HR_INSPECAO); df.ANOMALIA_FALHA=df.ANOMALIA_FALHA.map(txt); df.D1=df.D1.map(txt); df['D1_GERAL']=df.apply(lambda r: falha_geral(r.D1,r.ANOMALIA_FALHA),axis=1)
...
def load(c):
    ...
    if df.empty: return df
    df.DT_HR_INSPECAO=pd.to_datetime(df.DT_HR_INSPECAO,errors='coerce'); df.D1=df.D1.fillna('').astype(str); df.D1_GERAL=df.apply(lambda r: txt(r.D1_GERAL) or falha_geral(r.D1,r.ANOMALIA_FALHA),axis=1); return enrich(df.drop_duplicates())
...
def pareto_svg(p,title):
    ...
    for k in range(6):
        q=mx*k/5; y=yq(q); pct=k/5; parts += [f"<line x1='{ml}' y1='{y:.1f}' x2='{w-mr}' y2='{y:.1f}' stroke='#E2E5E9'/>",f"<text x='{ml-12}' y='{y+4:.1f}' fill='#657080' font-size='12' text-anchor='end'>{int(round(q))}</text>",f"<text x='{w-mr+12}' y='{yp(pct)+4:.1f}' fill='#657080' font-size='12'>{int(pct*100)}%</text>"]
...
def show_pareto_table(p):
    out=p.copy()
    if not out.empty:
        out.insert(0,'TOP',range(1,len(out)+1)); out['Percentual']=out.Percentual.map(fmt_pct); out['Percentual Acumulado']=out['Percentual Acumulado'].map(fmt_pct)
...
def score_pareto(dfcrit):
    if dfcrit.empty: return pd.DataFrame(columns=['Item','Quantidade','Percentual','Percentual Acumulado'])
    p=dfcrit[['Falha','Score criticidade']].rename(columns={'Falha':'Item','Score criticidade':'Quantidade'}).head(15).copy(); total=p.Quantidade.sum(); p['Percentual']=p.Quantidade/total if total else 0; p['Percentual Acumulado']=p.Percentual.cumsum(); return p
def estabilidade_pareto(dfcrit):
    if dfcrit.empty: return pd.DataFrame(columns=['Item','Quantidade','Percentual','Percentual Acumulado'])
    p=dfcrit.Estabilidade.value_counts().reset_index(); p.columns=['Item','Quantidade']; total=p.Quantidade.sum(); p['Percentual']=p.Quantidade/total if total else 0; p['Percentual Acumulado']=p.Percentual.cumsum(); return p

c=con(); init(c)
st.markdown(f"<div class='agco-header'>{logo_html()}<div class='head-title'><h1>Pareto de Falhas QG09</h1><p>Versão {APP_VERSION}: V0.17 preservada + criticidade e estabilidade com Pareto clássico.</p><span class='badge'>V0.17 base</span><span class='badge'>Criticidade</span><span class='badge'>Estabilidade</span><span class='badge'>Pareto clássico</span></div></div>",unsafe_allow_html=True)
df_all=load(c)
    ...
    if up:
        try:
            ...
            if st.button('Salvar no calendário',type='primary') and not falhas.empty:
                save(c,up.name,full,qg09,falhas,'Somar ao calendário'); st.rerun()
...
if df_all.empty or year is None:
    ...
    ydf,start,end,label=period(df_all,year,mode); modelos=sorted(df_all.MODELO_CORRIGIDO.dropna().unique()); origens=sorted(df_all.POSTO_ORIGEM_FALHA.dropna().unique()); areas=sorted(df_all.C_AREA_ORIGEM_FALHA.dropna().unique())
    ...
    with tabs[1]:
        if pg.empty: st.info('Sem dados.')
            ...
            bm=pareto(df_top,'MODELO_CORRIGIDO',25); st.markdown(pareto_svg(bm,'Distribuição por modelo'),unsafe_allow_html=True); show_pareto_table(bm.rename(columns={'Item':'Modelo'}))
            mod=st.selectbox('Escolha o modelo para ver as informações completas',['Todos']+sorted(df_top.MODELO_CORRIGIDO.dropna().unique()))
            dfm=df_top.copy() if mod=='Todos' else df_top[df_top.MODELO_CORRIGIDO.eq(mod)].copy(); det=pareto(dfm,'ANOMALIA_FALHA',50).rename(columns={'Item':'Falha completa'})
            st.markdown('### Ranking das falhas completas'); show_pareto_table(det)
            if not det.empty:
            ...
            st.markdown('### Registros completos'); st.dataframe(dfm[['DT_HR_INSPECAO','MODELO_CORRIGIDO','REGIAO_EXTRAIDA','FAMILIA_DEFEITO','D1_GERAL','ANOMALIA_FALHA','POSTO_ORIGEM_FALHA']],use_container_width=True,hide_index=True)
    with tabs[2]:
        mp=sorted(filt.MODELO_CORRIGIDO.dropna().unique())
        if not mp: st.info('Sem modelos disponíveis.')
        else:
            m=st.selectbox('Modelo para Pareto',mp); pm=pareto(filt[filt.MODELO_CORRIGIDO.eq(m)],'D1_GERAL',top_n); st.markdown(pareto_svg(pm,f'Pareto por Modelo - {m} - {label}'),unsafe_allow_html=True); show_pareto_table(pm)
    with tabs[3]:
        pr=pareto(filt,'REGIAO_EXTRAIDA',top_n); st.markdown(pareto_svg(pr,f'Pareto por Região - {label}'),unsafe_allow_html=True); show_pareto_table(pr.rename(columns={'Item':'Região'}))
        if not filt.empty:
            reg=st.selectbox('Escolha uma região para detalhar',sorted(filt.REGIAO_EXTRAIDA.dropna().unique())); dfr=filt[filt.REGIAO_EXTRAIDA.eq(reg)]; c1,c2=st.columns(2)
            with c1: st.markdown(pareto_svg(pareto(dfr,'D1_GERAL',10),f'Falhas na região: {reg}'),unsafe_allow_html=True)
            with c2: st.markdown(pareto_svg(pareto(dfr,'MODELO_CORRIGIDO',10),f'Modelos na região: {reg}'),unsafe_allow_html=True)
    with tabs[4]:
    ...
    with tabs[5]:
        fal=st.selectbox('Falha geral', sorted(filt.D1_GERAL.dropna().unique()) if not filt.empty else []); mod_t=st.selectbox('Modelo para tendência',['Todos']+sorted(filt.MODELO_CORRIGIDO.dropna().unique()))
        dft=filt[filt.D1_GERAL.eq(fal)].copy(); dft=dft if mod_t=='Todos' else dft[dft.MODELO_CORRIGIDO.eq(mod_t)]
        trend=dft.groupby('SEMANA_INICIO').size().reset_index(name='Quantidade').sort_values('SEMANA_INICIO'); st.line_chart(trend.set_index('SEMANA_INICIO')['Quantidade'] if not trend.empty else pd.Series(dtype=int)); st.dataframe(trend,use_container_width=True,hide_index=True)
    with tabs[6]:
        if filt.empty: st.info('Sem dados.')
        else:
            falha_ad=st.selectbox('Falha para comparar',['Todas']+sorted(filt.D1_GERAL.dropna().unique())); min_d,max_d=filt.DT_HR_INSPECAO.dt.date.min(),filt.DT_HR_INSPECAO.dt.date.max(); acao=st.date_input('Data da ação',value=max_d,min_value=min_d,max_value=max_d,format='DD/MM/YYYY')
            base=filt.copy() if falha_ad=='Todas' else filt[filt.D1_GERAL.eq(falha_ad)]; antes=base[base.DT_HR_INSPECAO.dt.date<acao]; depois=base[base.DT_HR_INSPECAO.dt.date>=acao]
            k1,k2=st.columns(2); k1.markdown(kpi('Antes',fmt_int(len(antes))),unsafe_allow_html=True); k2.markdown(kpi('Depois',fmt_int(len(depois))),unsafe_allow_html=True); st.bar_chart(pd.Series({'Antes':len(antes),'Depois':len(depois)}))
    with tabs[7]:
        pf=pareto(filt,'FAMILIA_DEFEITO',top_n); st.markdown(pareto_svg(pf,f'Pareto por Família de Defeito - {label}'),unsafe_allow_html=True); show_pareto_table(pf.rename(columns={'Item':'Família'}))
    with tabs[8]:
        st.markdown("<div class='panel'><b>Índice de criticidade da falha + Controle de estabilidade</b><br>Esta aba foi adicionada sobre a base da V0.17, mantendo as abas e gráficos anteriores.</div>",unsafe_allow_html=True)
        crit=criticidade(filt)
        k1,k2,k3,k4=st.columns(4); k1.markdown(kpi('Falhas avaliadas',fmt_int(len(crit))),unsafe_allow_html=True); k2.markdown(kpi('Críticas',fmt_int((crit.Criticidade=='Crítica').sum()) if not crit.empty else 0),unsafe_allow_html=True); k3.markdown(kpi('Em crescimento',fmt_int((crit.Estabilidade=='Em crescimento').sum()) if not crit.empty else 0),unsafe_allow_html=True); k4.markdown(kpi('Instáveis',fmt_int((crit.Estabilidade=='Instável').sum()) if not crit.empty else 0),unsafe_allow_html=True)
        st.markdown('### Pareto clássico por score de criticidade')
        pcrit=score_pareto(crit); st.markdown(pareto_svg(pcrit,'Pareto de Criticidade da Falha'),unsafe_allow_html=True); show_pareto_table(pcrit)
        st.markdown('### Pareto clássico por status de estabilidade')
        pest=estabilidade_pareto(crit); st.markdown(pareto_svg(pest,'Pareto de Estabilidade'),unsafe_allow_html=True); show_pareto_table(pest)
